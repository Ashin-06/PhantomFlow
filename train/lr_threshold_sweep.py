...
def main():
    print("=" * 80)
    print("LR DECISION VALUE THRESHOLD SWEEP FOR CROSS-FAMILY GENERALIZATION")
    print("=" * 80)
    ...
    for name, path_dict in OOD_SCENARIOS.items():
        ...
        ood_datasets[name] = (X_scaled, y_ood)

    # Perform decision value sweep
    # We will check thresholds from -10 to +2
    decision_thresholds = [-10.0, -5.0, -3.0, -1.0, 0.0, 1.0, 2.0]
    
    rows = []
    for t in decision_thresholds:
        # Convert decision value threshold to uncalibrated probability
        prob_thresh = 1.0 / (1.0 + np.exp(-t))
        
        for name, (X_scaled, y_ood) in ood_datasets.items():
    ...
    df_pivot = df_res.pivot_table(
        index=["Threshold (z)", "Probability (P)"],
        columns="OOD Family",
        values=["Recall", "Precision", "F1-Score"]
    )
    pd.set_option('display.max_columns', None)
    pd.set_option('display.width', 1000)
    print("\n" + "=" * 80)
    print("DECISION VALUE THRESHOLD SWEEP RESULTS")
    print("=" * 80)
    print(df_pivot)
    print("=" * 80)
