...
class Database:
    ...
    async def save_flow(self, flow: dict, features: Any = None) -> None:
        ...
        async with self.pool.acquire() as conn:
            ...
            existing = await conn.fetchval("SELECT 1 FROM flows WHERE flow_id = $1", flow.get("uid"))
            if existing:
                return
            ...
            """,
                flow.get("uid"),
                dt,
                flow.get("src_ip", flow.get("src", "0.0.0.0")),
                flow.get("dst_ip", flow.get("dst", "0.0.0.0")),
                flow.get("src_port", flow.get("sport")),
                flow.get("dst_port", flow.get("dport")),
                flow.get("proto", "TCP"),
                flow.get("server_name", flow.get("sni")),
                flow.get("ja3"),
                features.duration_s if (features and hasattr(features, "duration_s")) else flow.get("duration_s"),
                features.orig_bytes if (features and hasattr(features, "orig_bytes")) else flow.get("orig_bytes", 0),
                features.resp_bytes if (features and hasattr(features, "resp_bytes")) else flow.get("resp_bytes", 0),
                features.bytes_ratio if (features and hasattr(features, "bytes_ratio")) else None,
                features.iat_mean_ms if (features and hasattr(features, "iat_mean_ms")) else None,
                features.iat_cv if (features and hasattr(features, "iat_cv")) else None,
                features.periodicity_score if (features and hasattr(features, "periodicity_score")) else None,
                features.dominant_period_ms if (features and hasattr(features, "dominant_period_ms")) else None,
                features.dns_query if (features and hasattr(features, "dns_query")) else flow.get("dns_query"),
                features.dns_shannon_entropy if (features and hasattr(features, "dns_shannon_entropy")) else None,
            )
